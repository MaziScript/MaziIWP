...
var MaziScript = function (options) {
    ...
    var escape = function () {
        ...
        (function () {
            for (var i in chapterData.content.text) {
                ...
                if (chapterData.content.append[i] == "　　=>undefined") continue;
    ...
    var parseCommand = function (callback) {
         ...
         * 单条语句的语法规范
         * 
         * 单条语句是语句块的简略形式，但现在语句块的识别只有一种功能，即没有经过语法分析这一部分。
         * 因此单条语句的处理在此处只有一种形式。
         * 
         * 功能1：characterName(prop1:value1,prop2:value2,...,propN:valueN)
         */
        for (i in chapterData.command) {
            ...
            console.log(chapterData.command[i]);
            console.log(i);

            switch (type) {
                case 1:
                    if (regex.command_test.test(chapterData.command[i])) {
                        (function () {
                            var _title = regex.command_title.exec(chapterData.command[i])[1];
                            var content = regex.command_title.exec(chapterData.command[i])[2];

                            for (var i = 0; i <= content.match(regex.command_content).length; i++) {
                                var tmp = regex.command_content.exec(content);
                                for (var j in chapterData.dataJson) {
                                    if (chapterData.dataJson[j].name == parseObj) {
                                        parseObj = chapterData.dataJson[j];
                                    }
