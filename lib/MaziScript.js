...
var MaziScript = function(options){
    ...
    var defaults = {
        ...
        //段首空格规范
        paragraphPrefix : true,
        paragraphPrefixNum : 2
    };
    ...
    var horizontal = function(){
        ...
        for(var i = 1 ; i <= matchSharp.length ; i++){
            if(i > matchSharp.length - 1){
                chapterData.horizontalPart.push(utils.removeCrlf(chapterData.handle.substring(matchSharp[i-1] + position[i-1])));
            }
            else{
                chapterData.horizontalPart.push(utils.removeCrlf(chapterData.handle.substring(matchSharp[i] + position[i-1], position[i])));
            }
        }

        /*
        var pre = 0, tpos = 0, sharp = 0;
        chapterData.handle = chapterData.handle.replace(regex.horizontal, function(match, pos, originalText){
            chapterData.horizontalPart.push(
                utils.removeCrlf(function(){
                    var ans = originalText.substring(pre, pos);
                    console.log(ans);
                    pre = pos + match.length;
                    tpos = pos;sharp += match.length - 2;
        ...
        chapterData.horizontalPart.push(
            utils.removeCrlf(chapterData.handle.substring(tpos-sharp))
        );

        //去除因算法问题多余生成的空项
        chapterData.horizontalPart.shift();
        */
    }
        ...
        (function(){
            for(var i in chapterData.horizontalPart){
                ...
                if(flagType == 1){
                    (function(){
                        //区分是否存在作者留言
                        var append = typeof text.match(regex.contentAppend) == "object";
