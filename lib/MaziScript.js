            ...
            switch (type) {
                case 1:
                    if (regex.command_test.test(chapterData.command[i])) {
                        (function (i) {
                            var arr = regex.command_title.exec(chapterData.command[i]);
                            var _title = arr[1];
                            var content = arr[2];

                            var arr_content = content.match(regex.command_content);
                            for (var i = 0; i <= arr_content.length-1; i++) {
                                var tmp = regex.command_content.exec(arr_content[i]),
                                    parseObj = _title;
                                for (var j in chapterData.dataJson) {
                                    if (chapterData.dataJson[j].name == parseObj) {
                                        parseObj = chapterData.dataJson[j];
